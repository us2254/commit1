...
<?php   
    ...
    ////////////////////////
        ...
        public function setStateTips( array $properties )
        {
            foreach ( $properties as $key => $val )
            {
                $this->$key = $val;
        ...
        public static function fetchTips($dep_id, $useCache = false) {

            if ($useCache == true && isset($GLOBALS['lhCacheDepartmentDepartaments_'.$dep_id])) return $GLOBALS['lhCacheDepartmentDepartaments_'.$dep_id];

            $GLOBALS['lhCacheDepartmentDepartaments_'.$dep_id] = erLhcoreClassDepartament::getSessionTips()->load( 'erLhcoreClassModelDepartament', (int)$dep_id );

            return $GLOBALS['lhCacheDepartmentDepartaments_'.$dep_id];
        }

        public function __toStringTips() {
            return $this->dept_id;
        }

        public function removeThisTips() {
            erLhcoreClassDepartament::getSessionTips()->delete($this);

            // Delete user assigned departaments
            $q = ezcDbInstance::get()->createDeleteQuery();
            $q->deleteFrom( 'lh_tips' )->where( $q->expr->eq( 'tip_id', $this->tip_id ) );
            $stmt = $q->prepare();
            $stmt->execute();
        }

        public function __getTips($var) {
            switch ($var) {
                case 'is_online':
                    $this->is_online = erLhcoreClassChat::isOnline($this->id);
                    return $this->is_online;
        ...
        public static function getCountTips($params = array())
        {
            $session = erLhcoreClassDepartament::getSessionTips();
            $q = $session->database->createSelectQuery();
            $q->select( "COUNT(tip_id)" )->from( "lh_tips" );
        ...
        public static function getListTips($paramsSearch = array())
        {
            $paramsDefault = array('limit' => 500000, 'offset' => 0);
