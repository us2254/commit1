...
lhcAppControllers.controller('LiveHelperChatCtrl',['$scope','$http','$location','$rootScope', '$log','$interval','LiveHelperChatFactory', function($scope, $http, $location, $rootScope, $log, $interval,LiveHelperChatFactory) {
    ...
    this.getLimitChat = function () {
        this.limitc = this.restoreLocalSetting('limitc', 0, true);

        return this.limitc;
    };

    this.getLimitAssign = function () {
        this.limitd = this.restoreLocalSetting('limitd', 0, true);

        return this.limitd;
    };

    
    this.loggedUsers = function (user_id) {
        this.limitd = this.restoreLocalSetting('limitd', 0, true);
        this.limitc = this.restoreLocalSetting('limitc', 0, true);
        if (this.limitd == 0) {
            this.limitd = user_id;
            this.limitc = 0;
            _that.storeLocalSetting('limitd', this.limitd);
            _that.storeLocalSetting('limitc', this.limitc);
        }
        else {
            var found = false;
            this.limitd.forEach(function (entry) {
                if (entry == user_id) {
                    found = true;
                }
            });
            if (!found) {
                ...
                this.limitc.forEach(function (entry) {
                    arrayc[j] = entry
                    j++;
                });
                array[i] = user_id;
                arrayc[j] = 0;
                _that.storeLocalSetting('limitd', array);
                _that.storeLocalSetting('limitc', arrayc);
            }
        }
    ...
    this.startChat = function (chat_id, name) {
            ...
            if ($('#tabs').size() > 0) {
                return lhinst.startChat(chat_id, $('#tabs'), name);
            } else {
    ...
    this.increaseChatCount = function (user_id) {
        ...
        this.limitd.forEach(function (entry) {
            ...
            if (entry == user_id) {
                _that.limitc = _that.restoreLocalSetting('limitc', 0, true);
                _that.limitc[cou]++;
    ...
    this.start = function (user_id) {

        _that.limitd = this.restoreLocalSetting('limitd', 0, true);
        _that.limitc = _that.restoreLocalSetting('limitc', 0, true);
        var min = _that.limitd[0];
        var cou = 0;
        var sou = 0;
        _that.limitc.forEach(function (entry) {
    ...
    this.onlineOps = function (online) {
        alert(online);
    };
    $scope.clicki = 1;
    this.setClick = function (cl) {
        $scope.clicki = cl;
